...
> **Firmware note:** The ESP32 firmware applies a 50 ms hardware contact-bounce filter on the falling edge (non-blocking). Each physical button press emits exactly one `SOS_TRIGGER_RANDOM_MESSAGE` frame over Bluetooth SPP. All higher-level timing (1 500 ms double-tap window, 10-min cooldown) is handled by `IotSosTracker` on the Android side.
...
> **`alert_id` is now optional.** If omitted, the backend looks up the
> caller's most-recent active countdown alert and cancels it automatically.
> This handles the edge case where the app navigated to the home screen during
> the countdown (clearing `IotSosTracker.currentAlertId`) before the
> double-tap cancel was processed — without this fallback the cancel would
> fail with a validation error and `/send-now` would still dispatch the SOS.

**Success response `200` (alert found and cancelled)**

```json
{
  "status": "success",
  "data": { "alert_id": "abc123", "alert_status": "cancelled" }
}
```

**Success response `200` (no active countdown — already cancelled/sent)**
...
|---|---|
| Hardware contact-bounce filter (50 ms) | ESP32 firmware (`loop()`, edge-triggered, non-blocking) |
| Double-tap detection (1 500 ms window) | `IotSosTracker` (Android) |
...
| `/api/device/button-event` endpoint | **Not used** — app calls `/api/sos/trigger`, `/api/sos/cancel`, `/api/sos/safe` directly |
...
- [x] **`POST /api/device/register` is reachable and returns `200`** *(critical — app won't connect without this)*
...
- [x] **`"iot_button"` is accepted by `POST /api/sos/trigger`** *(critical — trigger is rejected silently if not allowlisted)*
- [x] **`accuracy: null` field in trigger body is accepted without error** *(fixed: `SOSTriggerSchema` now sets `Meta.unknown = EXCLUDE` and declares `accuracy` as an explicit nullable field — marshmallow 3 rejects unknown fields by default)*
- [x] Confirm `trigger_type` is persisted in the SOS alert record
- [x] **`POST /api/sos/trigger` with `trigger_type: "iot_button"` no longer hits the 20-second manual-SOS cooldown** — `IotSosTracker` owns the hardware cooldown; the backend was erroneously applying an additional in-process guard that blocked re-triggering after a cancel.
- [x] **`POST /api/sos/cancel` now works without `alert_id`** — if the field is omitted, the backend looks up the caller's most-recent active countdown alert and cancels it. This prevents silent dispatch when the app navigates home before the double-tap cancel clears `IotSosTracker.currentAlertId`.
- [x] **Manual SOS cooldown is cleared on cancel** — after a successful cancel the 20-second in-process guard is removed so manual SOS can be re-triggered immediately.
- [x] Confirm `POST /api/sos/cancel` enforces alert ownership (already hardened per safety layer doc)
- [x] Confirm `POST /api/sos/safe` works for alerts in both `countdown` and `sent` states
- [x] **`GET /api/sos/history` auto-expires stale `countdown` alerts** — alerts that never reached `/send-now` (app lost state) are marked `cancelled` with `resolution_type: "expired"` so history is accurate.
- [x] **Migration `g1h2i3j4k5l6`** — new migration that re-ensures `'iot_button'` is in `trigger_type_enum` using psycopg2 autocommit, safe on all PostgreSQL versions (the original migration in `c1d2e3f4g5h6` ran inside an Alembic transaction, which silently fails on PostgreSQL < 12 and would cause every IoT trigger to fail at DB commit).
- [x] Confirm `GET /api/sos/history` returns `trigger_type` in each record
