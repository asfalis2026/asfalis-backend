...
CSV_PATH = os.path.join(PROJECT_ROOT, "data_visualisation", "MEDIUM_DANGER.csv")
OUTPUT_PATH = os.path.join(PROJECT_ROOT, "danger_motion_distances.txt")
...
def calculate_distances():
    ...
    # The actual danger events are the high spikes. 
    # We want to remove the low stationary "safe" data from this danger dataset 
    # so we can find the true average of the danger impacts.
    # We use a threshold of 0.2 (which is safely above the safe motion average of 0.01-0.08)
    # to isolate only the vigorous/danger events.
    ...
    output_lines.append(f"Total High-Spike Danger Groups Retained: {len(filtered_means)}")
    ...
    output_lines.append("\n" + "="*50)
    output_lines.append(f"FINAL AVERAGE DANGER DISTANCE (spikes only): {final_average:.6f}")
